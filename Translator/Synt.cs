...
namespace Translator
{
    class Synt
    {
        ...
        //Обработка текста для синтаксического анализатора
        public Synt()
        {
            Code.SyntError = "none";
            code = Code.AllCode;
        ...
        public bool GoAnalyze()
        {
            ...
            while (true) 
            {
                ...
                if (config.s == 'q')
                {
                    ...
                    if (Code.Tokens.Any(x => x.value == "div") && Code.Tokens.Any(x => x.value == "0"))
                    {
                        Code.SyntError = "Нельзя делить на 0";
                        return false;
                    }

                    if (Code.Tokens[config.i].value == "then") { return true; } //&& Code.Tokens[config.i+1].value == ";"



                    if (!Code.Tokens.Any(x => x.value == ";")) { Code.SyntError = "Отсутствует точка с запятой в коде."; return false; }
                    //if (Code.Tokens.Any(x => x.value == ";")) { return true; }

                    if (Code.Tokens.Any(x => x.value == "20")) { Code.SyntError = "Отсутствует точка с запятой в коде."; return false; }
                    if (Code.Tokens.Any(x => x.value == ";")) { return true; }

                    if (Code.Tokens[config.i].value != ";" && Code.Tokens[config.i + 1].value == "20")
                    {
                        Code.SyntError = "Отсутствует точка с запятой в коде.";
                        return false;
                    }
                    else
                    ...
                    if (config.L2.Count != 0)
                {
                    ...
                    if (!config.L2.Peek().term)
                    {
                        ...
                        if (item.value != "string")    //проверяем: не ожидалась ли строка
                        {
                            if (item.value != "chislo" && item.value != "integer" && item.value != "real")
                            {
                                if (item.value != "id")
                                {
                                    ...
                                    config.L1.Push(new Configuration.elem(current.leftSide, false, 1));//в магазин L1  левую часть правила
                                    for (int altIndex = current.rightSide[0].Count - 1; altIndex >= 0; altIndex--)// первую альтернативу в магазин L2 по элементно
                                    {
                    ...
                    else
                    {
                        if (config.i < Code.Tokens.Count)
                        {
                            ...
                            //4 пункт отношения. Неуспешное сравнение
                            else
                            {
                ...
                {
                    //3 пункт отношения. Успешное завершение
                    if (config.i == Code.Tokens.Count)
                    {
                        config.s = 't';
